...
#declare -a addresses=("172.19.2.115" "172.19.2.107")
threshold=$2
...
for t in ${techniques}; do
  ...
  for i in `seq 0 ${y}`; do
    ...
    ssh roott@${a} 'bash -s' < runWorkloadWatDiv.sh ${b} 4 ${t}-client-eval ${a} ${n} ${c} ${threshold} ${t}  > outputRunWorkloadWatDiv_${t}_${a}_${b}_${c} &
    pid=$!
    spids="$spids $pid"
    b=$(($b+1))
  done
