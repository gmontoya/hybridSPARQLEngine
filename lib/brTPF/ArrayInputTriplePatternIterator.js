...
ArrayInputTriplePatternIterator.prototype._readTransformer = function (fragment, fragmentBindings) {
  ...
  while (triple = fragment.read()) {
    ...
    fragmentBindings.forEach( function (solMap, index) {
      try {
        //console.log('ArrayI*:45, solMap: '+JSON.stringify(solMap));
        var extendedBindings = rdf.extendBindings(solMap, this._pattern, triple);
        //console.log('ArrayInputTriplePatternIterator:47, pushing '+JSON.stringify(extendedBindings));
