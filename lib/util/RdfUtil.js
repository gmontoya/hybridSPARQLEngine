...
util.convertToString = function (triple) {

    if (!triple)
      return '';
    // Convert a triple component by abbreviating it
    if (typeof triple === 'string') {
      if (util.isVariable(triple))
        return triple;
      if (util.isLiteral(triple)) {
        var litLan = util.getLiteralLanguage(triple);
        var litType = util.getLiteralType(triple);
        var litValue = '"' + util.getLiteralValue(triple) + '"';
        //console.log('litValue: '+litValue+'. litLan: '+litLan+'. litType: '+litType);
        if (litLan && litLan.length>0) {
          litValue = litValue+"@"+litLan;
        } else if (litType && litType.length>0) {
          litValue = litValue + "^^<"+litType+">";
        }
        return litValue;
        //return '"' + util.getLiteralValue(triple) + '"';
      }
      if (util.isIRI(triple)) {
        return '<'+triple+'>';
      }
      //var match = triple.match(/([a-zA-Z\(\)_\.,'0-9]+)[^a-zAZ]*?$/);
      return triple; //match ? match[1] : triple;
    }
    // Convert a triple by converting its components
    return util.convertToString(triple.subject) + ' ' +
           util.convertToString(triple.predicate) + ' ' +
           util.convertToString(triple.object) + '.';
}


util.convertFromJSON = function (result) {
...
util.extendBindings = function (bindings, triplePattern, boundTriple) {
  ...
  //console.log('RdfUtil:184, extendBindingsBindings, bindings: '+JSON.stringify(bindings));
  //console.log('RdfUtil:185, extendBindingsBindings, newBindings: '+JSON.stringify(newBindings));
  //console.log('RdfUtil:186, extendBindingsBindings, triplePattern: '+JSON.stringify(triplePattern));
  //console.log('RdfUtil:187, extendBindingsBindings, boundTriple: '+JSON.stringify(decodeURIComponent(boundTriple.subject)));
...
//  util.addBinding(newBindings, triplePattern.subject,   decodeURIComponent(boundTriple.subject));
//  util.addBinding(newBindings, triplePattern.predicate, decodeURIComponent(boundTriple.predicate));
//  util.addBinding(newBindings, triplePattern.object,    decodeURIComponent(boundTriple.object));
  return newBindings;
};

/** Creates augmented bindings that include bindings obtained from the evaluation of a BGP by a SPARQL endpoint */
util.extendBindingsBindings = function (bindings, moreBindings) {
  var newBindings = Object.create(null);
  for (var binding in bindings) {
    newBindings[binding] = bindings[binding];
  }
  //console.log('RdfUtil:198, extendBindingsBindings, bindings: '+JSON.stringify(bindings));
  //console.log('RdfUtil:199, extendBindingsBindings, newBindings: '+JSON.stringify(newBindings));
  //console.log('RdfUtil:199, extendBindingsBindings, moreBindings: '+JSON.stringify(moreBindings));
  for (var binding in moreBindings) {
    //var binding = moreBindings[i];
    //console.log('RdfUtil:203, extendBindingsBindings, binding: '+binding);
    var value = moreBindings[binding];
    //console.log('RdfUtil:205, extendBindingsBindings, binding: '+binding+'. value: '+value);
    /*if ((typeof value) === 'object') {
        value = value['value'];
        //console.log('RdfUtil:149, extendBindingsBindings, value was an object now is: '+value);
    }*/
    util.addBinding(newBindings, binding, value);
  }
  return newBindings;
};
...
util.addBinding = function (bindings, left, right) {
  //console.log('RdfUtil:155, addBinding, left: '+left+', right: '+right);
