...
function SPARQLEndpointClient(fragmentsClient, options) {
    ...
    this.fragmentsClient = fragmentsClient;
...
SPARQLEndpointClient.prototype.evaluate = function (query) {
  ...
  function performRequest() {
    ...
    self.fragmentsClient._numberOfSPARQLEndpointCalls++;
    //console.log('SPARQLEndpointClient:29, self.fragmentsClient._numberOfSPARQLEndpointCalls: '+self.fragmentsClient._numberOfSPARQLEndpointCalls);
    ...
    try { request = self._request(requestOptions); }
      ...
      /*console.log('SPARQLEndpointClient:41,error.stack: '+error.stack);*/ 
      return setImmediate(emitError, error), responseIterator; }
    ...
    request.on('response', function (response) {
      ...
      if (statusCode !== 200) {
        responseIterator.emit('error', new Error('Endpoint could not evaluate query ('+response.statusCode+', '+response.statusMessage+')'));
      }
      // Start a possible pending request
      self.fragmentsClient._HTTPTimeSE = self.fragmentsClient._HTTPTimeSE + responseTime;
      self._activeRequests--;
