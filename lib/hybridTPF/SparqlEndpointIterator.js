...
SparqlEndpointIterator.prototype._createTransformer = function (solMap, options) {
  ...
  function extractVariables(variables, bgp) {
      ...
      variables.forEach(function(item) {
          ...
          if (vars.includes && vars.includes(aux)) {
              str = str + aux + ' ';
          }
      });
  ...
  function extractVariablesB(variables, bgp) {
      var vars = [];
      //console.log('SparqlEndpointIterator:130, vars: '+vars);
      for (var i = 0; i < bgp.length; i++) {
          var t = bgp[i];
          vars = vars.concat(rdf.getVariables(t));
      }
      //console.log('SparqlEndpointIterator:135, vars: '+vars);
      var vs = [];
      variables.forEach(function(item) {
          //console.log('SparqlEndpointIterator:138, item: '+item);
          var aux = item;
          if (aux.charAt(0) !== '?') {
              aux = '?'+aux;
          }

          //console.log('SparqlEndpointIterator:140, vars.includes(aux): '+vars.includes(aux));
          if (vars.includes && vars.includes(aux)) {
              vs.push(aux);
          }
      });
