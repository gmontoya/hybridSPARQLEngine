...
/* A hybridTPFClient combines fetching brTPFs through HTTP and evaluating SPARQL Queries at endpoints. */

var HttpClient = require('../util/HttpClient'),
    SPARQLEndpointClient = require('./SPARQLEndpointClient');
...
// Creates a new hybridTPFClient
function hybridTPFClient(startFragment, options) {
  if (!(this instanceof hybridTPFClient))
    return new hybridTPFClient(startFragment, options);
  this._numberOfSPARQLEndpointCalls = 0;
  this._overallReceivedBytesEndpoint = 0;
  this._numberOfTTFromSPARQLEndpoint = 0;
  this._numberOfTriplePatternsInQuery = 0;
  this._overallNumberOfMatchingTriples = 0;
  this._overallNumberOfTuplesReceived = 0;
  this._overallNumberOfTriplesReceived = 0;
  this._overallReceivedBytesTPFServer = 0;
  //this._overallReceivedBytesEndpoint = 0;
  this._HTTPTimeSE = 0;
...
this._prefixes = {};
  ...
  this._sparqlEndpointClient = new SPARQLEndpointClient(this, options);
...
hybridTPFClient.prototype.getFragmentByPattern = function (tp) {
  return this.getFragmentByPatternAndSolMaps( tp, null );
}

// Returns the brTPF for the given triple pattern and set of solution mappings
hybridTPFClient.prototype.getFragmentByPatternAndSolMaps = function (tp, solMapSet) {
...
//                  projectOutVariablesFromSolMapSet( solMapSet, tp ); // no canonicalization
  ...
  startFragment.getProperty('controls', function (controls) {
    ...
    //console.log('hybridTPFClient:130, JSON.stringify(canonicalizedTP): '+JSON.stringify(canonicalizedTP));
...
// return removeDuplicatesFromSolMapSet( solMapSetReduced ); // unsorted
  ...
  function getFragmentUrlExtension( solMapSet, prefixes ) {
    ...
    //console.log('hybridTPFClient:351, values: '+values);
...
//      Logger.warning( item );
      ...
      if (this._fragmentsClient._countBytes) {
          this._fragmentsClient._overallReceivedBytesTPFServer += Buffer.byteLength(JSON.stringify(item), 'utf8');
      }
      this._push(item);
      //console.log("hybrid*:376, item: "+JSON.stringify(item));
    }
  }
};
  ...
  fragmentPage.getProperty('statusCode', function (statusCode) {
    ...
    fragmentPage.getProperty('contentType', function (contentType) {
      ...
      function loadNextPage() {
        ...
        var nextPage;
        try { nextPage = controls && controls.nextPage; } catch (controlError) { /*console.log(controlError.stack);*/ }
...
module.exports = hybridTPFClient;
