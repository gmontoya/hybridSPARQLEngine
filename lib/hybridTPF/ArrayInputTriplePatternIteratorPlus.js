...
function ArrayInputTriplePatternIteratorPlus(parent, bgp, chunkSize, options) {
  ...
  console.log('ArrayInputTriplePatternIteratorPlus:19, constructor, bgp: '+bgp.toString());
}
...
ArrayInputTriplePatternIteratorPlus.prototype._createTransformer = function (solMapSet, options) {
  var done = solMapSet['__d__'] ? true : false;
  ...
  fragment.getProperty('metadata', function (metadata) {
      ...
      if (metadata.totalTriples > self._options.threshold) {
        ...
        pipeline.setSource( new SparqlEndpointIterator(startIterator, self._pattern, self._chunkSize, self._options)); //new SparqlEndpointIterator(bgp, solMapSet);
...
ArrayInputTriplePatternIteratorPlus.prototype._readTransformer = function (fragment, fragmentBindings) {
  //console.log('ArrayInputTriplePatternIteratorPlus:62, _readTransformer,  fragmentBindings: '+ fragmentBindings);
    ...
    fragmentBindings.forEach( function (solMap, index) {
      try {
        if (fragment.isDone) {
            ...
            this._tmpChunk.push( rdf.extendBindingsBindings(solMap, anotherSolMap) );
        } else {
