...
var ArrayInputTriplePatternIterator = require('./ArrayInputTriplePatternIteratorPlus');
...
//              selectNextTP_MinMatchesAfterMinNumOfVarsUnboundAfterMaxNumOfVarsBound( bgp, varsCovered, bgpTotalTriples );
      ...
      pipeline = new ArrayInputTriplePatternIterator(pipeline, bgpCopy, chunkSize, options);
    }
    /*} else {
      var bgpCopy = [ firstTP ];
      bgpCopy = bgpCopy.concat(bgp);
      console.log('StaticBGPIterator:110, bgpCopy: '+bgpCopy);
      pipeline = new Iterator.PassthroughIterator(true);
      pipeline.setSource( new SparqlEndpointIterator(startIterator, bgpCopy, chunkSize, options)); //new SparqlEndpointIterator(bgp, solMapSet);
